...
function Focus({ state, onWorkOnThis }: { state: FocusState; onWorkOnThis: () => void }) {
  ...
  return <><section className="focus-hero" aria-labelledby="focus-heading"><p className="eyebrow">Focus <span aria-hidden="true">/</span> TAROKE RIMIXER</p><h1 id="focus-heading">{blocked ? 'A repair is needed before the next mission' : 'How can the remix preserve the poem’s internal rhythm?'}</h1><div className={blocked ? 'state-line is-blocked' : 'state-line'}><span aria-hidden="true">{blocked ? '!' : '●'}</span><span>{blocked ? 'Blocked — canonical records need repair' : 'Active inquiry · Evidence is being gathered'}</span></div><p className="objective"><span>Current objective</span>Establish a reproducible listening and annotation pass for the current verse set.</p></section><div className="focus-grid"><section className="focus-primary" aria-label="Current work"><article className="card next-action"><p className="card-label">{blocked ? 'Blocker' : 'Next action'}</p><h2>{blocked ? 'Repair the missing source reference' : 'Compare two rhythmic readings'}</h2><p>{blocked ? 'The source reference for the current question cannot be resolved. No mission can safely start until it is restored.' : 'Listen to the current cut beside the annotated source and identify what carries through.'}</p><button className="button button-primary" type="button" onClick={onWorkOnThis}>{blocked ? 'Open repair route' : 'Work on this'}</button></article><article className="card evidence-card"><p className="card-label">Evidence summary</p><p>3 source notes inspected · 1 rhythmic relation remains provisional</p><button className="text-action" type="button">Inspect evidence</button></article></section><aside className="focus-secondary" aria-label="Project orientation"><article className="card needs-you"><div><p className="card-label">Needs you</p><h2>2 items await review</h2></div><button className="text-action" type="button">Open review</button></article><article className="card"><p className="card-label">Latest accepted decision</p><h2>Keep the vocal trace audible</h2><p>Accepted yesterday · DEC-014</p></article><article className="card"><p className="card-label">Latest output</p><h2>Verse 3 listening cut</h2><p>Candidate audio · verification pending</p></article></aside></div></>;
}
